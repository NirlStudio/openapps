...
class NirlSQLi {
	...
	public function insert($types=NULL, $params=NULL, $longdata=NULL, $try=FALSE) {
        ...
        $result = isset($this->stmt->insert_id) ? $this->stmt->insert_id : TRUE ;
		$this->free() ;
		return $result ;
    }
	 ...
	 */
	public function delete($types=NULL, $params=NULL, $longdata=NULL) {
        
        if ( $this->innerExec('delete', $types, $params, $longdata) === FALSE )
			return FALSE ;
    ...
    private function bindParams($types, $params, $has_longdata) {
        ...
        if ( $count > MAX_PARAM_COUNT ) {
            ...
            $this->elog("exec::bindParams::too many parameters - $count.", $types, $params);
            return FALSE ;
        }
		
        if ( $count !== count($params) ) {
            
            $this->elog("exec::bindParams - $types does not match $params.", $types, $params);
            return FALSE ;
        }
