...
class NirlLog {
    ...
    private static $defaultsql = 
        'INSERT INTO t_log(log_time,log_level,log_type,log_mod,log_desc,log_data) 
         VALUES (now(), ?, ?, ?, ?, ?)';
    ...
    private static function config() {
        ...
        self::$level = isset($NirlLog_level) ? $NirlLog_level : 2;
        self::$save_data = isset($NirlLog_save_data) ? !empty($NirlLog_save_data) : TRUE;
        self::$max_data_length = isset($NirlLog_max_data_length) ? $NirlLog_max_data_length : 4096;
        self::$use_error_log = isset($NirlLog_use_error_log) ? !empty($NirlLog_use_error_log) : TRUE;
        
        self::$db = isset($NirlLog_db) ? $NirlLog_db : NULL;
        self::$sql = isset($NirlLog_sql) ? $NirlLog_sql : self::$defaultsql;
    }
    ...
    private static function save($level, $desc, $mod, $type, $data) {
        ...
        if ( empty(self::$db) || empty(self::$sql) ) // no to use database
            return;
        ...
        if ( !$db->query($sql) ) {
            ...
            $db->close();
            error_log('NirlLog::save::failed to set time_zone to UTC.');
            return;
        }
    ...
    /*
     * To record an error log - level 1
     */
    ...
    /*
     * To record a warning log - level 2
     */
    ...
    /*
     * To record an informational log. - level 3
     */
